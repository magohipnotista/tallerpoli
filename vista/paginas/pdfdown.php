...
<div class="modal-body text-center">
    <?php if (isset($_GET["pagina"])) : ?>
        <?php for ($i = 0; $i < count($registros); $i++) : ?>
            <?php if ($_GET["pagina"] == $registros[$i]["numerodeordendown"]) : ?>
                <?php
    ...
    <?php if (isset($_GET["pagina"])) : ?>
        <?php for ($i = 0; $i < count($facturas); $i++) : ?>
            <?php if ($_GET["pagina"] == $facturas[$i]["numerodeordendown"]) : ?>
...
if (isset($_GET["pagina"])) {
    for ($i = 0; $i < count($facturas); $i++) {
        if ($_GET["pagina"] == $facturas[$i]["numerodeordendown"]) {
            $dompdf->stream($facturas[$i]["nombrecliente"] . '-' . $facturas[$i]["numeroorden"] . ".pdf", array("Attachment" => true));
            echo $dompdf->output();
        }
    }
}
